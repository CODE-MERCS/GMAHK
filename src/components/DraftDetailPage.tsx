...
const formStructure = [
  ...
  },
  {
    ...
    items: [
      {
        key: "hadirSabat2",
        label: "(a). Jumlah Anggota Yang Hadir Sabat Ke-2 Dalam Triwulan Berjalan",
        validate: false,
      },
      {
        key: "hadirSabat7",
        label: "(b). Jumlah Anggota Yang Hadir Sabat Ke-7 Dalam Triwulan Berjalan",
        validate: false,
      },
      {
  ...
  },
  {
    ...
    label: "10. Jumlah Orang di Berikan Pembelajaran Alkitab Non SDA (Belum di Baptis)",
    validate: true,
    id: 9,
  },
  {
...
const DraftDetailPage = () => {
        ...
        if (response && response.data) {
          ...
          Object.entries(draftData).forEach(([key, value]) => {
            if (key !== 'id' && key !== 'createdAt' && key !== 'updatedAt' && 
                key !== 'userId' && key !== 'username' && !key.startsWith('foto')) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, key: string) => {
    if (e.target.files && e.target.files[0]) {
      setSelectedFiles(prev => ({ ...prev, [key]: e.target.files?.[0] || null }));
    }
  };

  // Get the image field name for a given data field
  const getImageFieldName = (fieldKey: string): string | null => {
    const mapping = Object.values(categoryMapping).find(m => m.field === fieldKey);
  ...
  const handleValidation = async (categoryId: number, fieldKey: string) => {
    ...
    try {
      ...
      const response = await validateDraftField(categoryId, parseInt(id, 10), formDataToSend);
  ...
  const handleSendDraft = async () => {
    ...
    try {
      ...
      if (error.response?.data?.invalidFields) {
        const fields = error.response.data.invalidFields.map((f: any) => f.field).join(", ");
    ...
    return (
      <div className="min-h-screen p-6">
        <div className="bg-green-100 p-8 rounded-lg text-center">
          ...
          <h2 className="text-2xl font-bold text-green-700 mb-4">Draft Berhasil Dikirim!</h2>
          <p className="text-green-600 mb-6">Draft laporan telah berhasil dikirim dan akan diproses oleh sekretaris.</p>
          <Link 
            to="/dashboard/draft" 
  ...
  if (error) {
    return (
      <div className="min-h-screen p-6">
        <Link to="/dashboard/draft" className="flex items-center text-green-600 mb-8">
  ...
  return (
    <div className="min-h-screen p-6">
      ...
      <div className="flex justify-between items-center mb-8">
        <Link to="/dashboard/draft" className="flex items-center text-green-600">
          <ArrowLeft className="mr-2" size={20} />
          Kembali ke Daftar Draft
        </Link>
        ...
        <button
          ...
          disabled={sending}
          className="flex items-center bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 transition-colors disabled:bg-gray-400"
        >
          {sending ? (
            <>
        ...
        <div className="flex items-start">
          ...
          <div className="ml-3">
            <p className="text-sm text-yellow-700">
              Pastikan semua field yang wajib divalidasi sudah terisi nilai dan memiliki bukti foto. Field wajib meliputi:
            </p>
                        ...
                        <td className="p-3 border">
                          {item.key === "bulan" ? (
                            <select
                              ...
                              disabled
                            >
                              <option value={formData.bulan}>{formatBulan(formData.bulan)}</option>
                            </select>
                          ) : (
                            <input
                              type="number"
                            ...
                            <div>
                              <button
                                ...
                                className={`${isValidated 
                                  ...
                                  : (!categoryMapping[item.id!]?.optional && 'bg-yellow-500 hover:bg-yellow-600') ||
